...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,x,masks):
        ...
        values = self.value_w(x).view(batch_size,self.max_seq_len,self.head_num,self.d_k).permute(0,2,1,3) # [batch_size, head_num, max_seq_len, d_v]
        ...
        temp = queries.matmul(keys.transpose(2,3))/self.d_k
        masks = masks.view(masks.size()[0],1,1,masks.size()[1]) # [batch_size,1,1,max_seq_len]
        # masks = masks.transpose(2,3) 
        temp=temp.masked_fill(masks==0,float('-inf'))
        scores=F.softmax(temp,3)
